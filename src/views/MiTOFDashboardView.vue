<!-- MiTOFDashboardView.vue - BBox 전용 대시보드 -->
...
<template>
  ...
  <div class="dashboard-header">
    <h1 class="header-title">MiTOF 대시보드 (BBox)</h1>
...
export default {
  ...
  data() {
    return {
      ...
      originalData: [],
      data: [],
      current: this.$store.getters.getMiTOFDashboardCurrentPage || 1,
      total: 0,
      lastPage: 0,
      itemsPerPage: 50,
      searchQuery: this.$store.getters.getMiTOFDashboardSearchHistory || "",
      isFocused: false,
      isExporting: false,
  ...
  watch: {
    searchQuery(newQuery) {
      this.updateMiTOFDashboardSearchHistory(newQuery);
    },
    current(newPage) {
      this.updateMiTOFDashboardCurrentPage(newPage);
    },
  },
  ...
  async loadDashboardData() {
      try {
        ...
        if (this.searchQuery) {
          this.assignMiTOFDashboardSearchFromStore();
        }

        if (this.$store.getters.getMiTOFDashboardCurrentPage) {
          this.current = Math.min(
            this.$store.getters.getMiTOFDashboardCurrentPage,
            this.lastPage
          );
        }
      } catch (error) {
  ...
  this.loadDashboardData();
    ...
    searchDashboard() {
      ...
      this.applyMatchSummaryFilter();
      this.current = 1;
      this.$store.commit("setMiTOFDashboardSearchHistory", this.searchQuery);
    },
    resetSearch() {
      this.searchQuery = "";
    ...
    downloadSearchedItems() {
      ...
      this.$axios
        ...
        .then((response) => {
          ...
          link.setAttribute(
            "download",
            `MiTOF-${this.searchQuery ? this.searchQuery : "전체과제"}-#${
              this.sliderValue
    ...
    assignMiTOFDashboardSearchFromStore() {
    ...
    updateMiTOFDashboardSearchHistory(history) {
      this.$store.commit("setMiTOFDashboardSearchHistory", history);
    },
    updateMiTOFDashboardCurrentPage(page) {
      this.$store.commit("setMiTOFDashboardCurrentPage", page);
    },
    async showMetrics() {
      ...
      try {
        this.isExporting = true;
        this.exportingMessage = "MiTOF Metrics 분석중입니다.";
        ...
        let alertMessage = "[ MiTOF (BBox) 과제들의 Metrics ]\n\n";
