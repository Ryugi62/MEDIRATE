  ...
  this.userSquaresList.forEach((user) => {
    user.squares = user.squares.map((square) => {
      if (square.isAI) {
        // 해당 square의 questionId에 해당하는 이미지의 크기를 찾아온다
        const relatedQuestion = this.data[0].questions.find(
          (question) => question.id === square.questionIndex
        );

        if (relatedQuestion) {
          // 해당 이미지의 실제 크기를 가져온다
          this.getImageDimensions(relatedQuestion.image).then((dimensions) => {
            const { width: imgWidth, height: imgHeight } = dimensions;

            // 원본 이미지 대비 현재 beforeCanvas의 비율을 계산하여 원본 이미지 기준인지 확인
            const widthRatio = user.beforeCanvas.width / imgWidth;
            const heightRatio = user.beforeCanvas.height / imgHeight;

            // 비율이 1이 아니면 원본 이미지 기준으로 계산된 좌표임
            if (widthRatio !== 1 || heightRatio !== 1) {
              const { x, y } = this.convertToBeforeCanvasCoordinates(
                square.x,
                square.y,
                imgWidth, // 원본 이미지의 width
                imgHeight, // 원본 이미지의 height
